...
func TestPool_PutUnusableConn(t *testing.T) {
	...
	conn.Close()
	if p.Len() != poolSize-1 {
		t.Errorf("Pool size is expected to be initial_size - 1 %v %v", p.Len(), poolSize-1)
	}
}
...
func newChannelPool() (Pool, error) {
	return NewGPool(InitialCap, MaximumCap, factory)
}
